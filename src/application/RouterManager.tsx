...
export class RouterManager {
  ...
  getRoutesTree(): RouteObject[] {
    type RouteTypeWithChildren = RouteType & {
      children?: RouteTypeWithChildren;
    };
    ...
    for (const [name, route] of Object.entries(this.routes)) {
      set(routes, name.split(".").join(".children."), {
        ...get(routes, name.split(".").join(".children.")),
        ...route,
      });
    }
  ...
  getRouterComponent(children?: React.ReactNode) {
    ...
    const RenderRouter: React.FC<{
      BaseLayout?: ComponentType<PropsWithChildren>;
