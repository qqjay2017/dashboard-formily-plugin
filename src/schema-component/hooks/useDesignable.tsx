...
export class Designable {
  ...
  on(
    name: "insertAdjacent" | "remove" | "error" | "patch" | "batchPatch",
    listener: any
  ) {
    if (!this.events[name]) {
      this.events[name] = [];
  ...
  async emit(
    ...
    return Promise.all(
      this.events[name].map((fn) =>
        fn.bind(this)({ current: this.current, ...opts }, ...others)
      )
    );
  }
  ...
  recursiveRemoveIfNoChildren(
    schema?: Schema,
    options?: RecursiveRemoveOptions
  ) {
    if (!schema) {
      return;
    }
  ...
  remove(schema?: Schema, options: RemoveOptions = {}) {
    ...
    if (removeParentsIfNoChildren) {
      const parent = this.recursiveRemoveIfNoChildren(s.parent, {
        breakRemoveOn,
      });
      if (parent) {
        removed = parent;
  ...
  insertBeforeBeginOrAfterEnd(
    ...
    return fromIndex > toIndex
      ? this.insertBeforeBegin(schema, options)
      : this.insertAfterEnd(schema, options);
  }
...
export function useDesignable() {
  const { designable, setDesignable, refresh, reset } = useContext(
